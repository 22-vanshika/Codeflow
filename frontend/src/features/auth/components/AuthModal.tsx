...
import { signInWithPopup, signInWithEmailAndPassword, createUserWithEmailAndPassword, sendPasswordResetEmail, GithubAuthProvider } from 'firebase/auth';
...
export default function AuthModal({ isOpen, onClose }: AuthModalProps) {
    ...
    const { setUser, setGithubAccessToken } = useAuthStore();
    ...
    const headerConfig = {
        login: { title: 'Welcome Back', subtitle: 'Sign in to sync your DSA progress' },
        signup: { title: 'Create Account', subtitle: 'Join CodeFlow to master your patterns' },
        forgotPassword: { title: 'Reset Password', subtitle: 'Enter your email to receive a reset link' },
    };
                    ...
                    >
                        ...
                        <div className="relative p-10">
                            ...
                            <div className="text-center mb-8">
                                ...
                                >
                                    {mode === 'forgotPassword' ? <KeyRound size={24} className="text-white" /> : <ShieldCheck size={24} className="text-white" />}
                                </motion.div>
                                <h2 className="text-3xl font-black text-text-primary mb-2 tracking-tight">
                                    {headerConfig[mode].title}
                                </h2>
                                <p className="text-text-secondary text-sm font-medium">
                                    {headerConfig[mode].subtitle}
                                </p>
                            </div>
                                    ...
                                    {resetSent ? (
                                        <div className="text-center py-6">
                                            ...
                                            <button
                                                ...
                                                className="text-primary hover:text-white font-bold text-sm transition-colors"
                                            >
                                                ← Back to Sign In
                                            </button>
                                        </div>
                                        ...
                                        <form onSubmit={handlePasswordReset} className="space-y-4">
                                            ...
                                            <button 
                                                type="submit"
                                                disabled={isLoading}
                                                className="group relative w-full py-4 px-6 rounded-2xl bg-primary text-white font-black transition-all mt-2 overflow-hidden shadow-lg shadow-primary/20 hover:shadow-primary/40"
                                            >
                                            ...
                                            <button
                                                type="button"
                                                onClick={() => handleModeChange('login')}
                                                className="w-full text-center text-sm text-text-secondary hover:text-white transition-colors font-bold mt-2"
                                            >
                                                ← Back to Sign In
                                            </button>
                                        </form>
                                ...
                                <>
                                            ...
                                            <div className="flex items-center justify-between ml-1">
                                                ...
                                                {mode === 'login' && (
                                                    <button
                                                        ...
                                                        className="text-xs text-primary hover:text-white transition-colors font-bold"
                                                    >
                                                        Forgot Password?
                                                    </button>
                                                )}
                                    ...
                                    <div className="mt-8 text-center">
                                        <p className="text-sm font-bold text-text-secondary">
                                            {mode === 'login' ? "New to CodeFlow? " : "Already have an account? "}
                                            <button 
