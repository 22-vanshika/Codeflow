...
export default function OutputPanel() {
    const { traces, currentStepIndex, isPlaying } = useExecutionStore();

    // Get output from current step or last available
    const currentTrace = traces[currentStepIndex];
    const output = currentTrace?.output || "";
    const isFinished = currentStepIndex === traces.length - 1 && traces.length > 0 && !isPlaying;
    ...
    if (!output && !isFinished) return null;
            ...
            {(isFinished || output) && (
                ...
                >
                    ...
                    <div className="p-4 font-mono text-sm text-text-muted overflow-auto whitespace-pre-wrap">
                        {output ? output : <span className="italic opacity-50">No output generated.</span>}
                        {isFinished && (
